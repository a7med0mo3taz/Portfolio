...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        {/* Logo */}
        <Link href="#home" className={`font-bold tracking-tighter transition-all duration-300 ${scrolled ? "text-xl" : "text-2xl"}`}>
        ...
        <nav className="flex items-center gap-10">
          {navLinks.map((link) => (
            ...
            >
              <span className={activeSection === link.name ? "text-white" : "text-gray-400 group-hover:text-white"}>
                {link.name}
              </span>
      ...
      {/* Mobile Header */}
      <div className={`lg:hidden fixed top-0 left-0 right-0 z-40 px-5 transition-all duration-300 flex justify-between items-center ${scrolled ? "bg-[#050505]/80 backdrop-blur-md border-b border-white/5 py-3" : "py-5"}`}>
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                ...
                {/* Icon */}
                <Icon className={`text-xl transition-all duration-300 ${isActive ? "text-primary -translate-y-5" : "text-gray-400"}`} />
                
                {/* Text Label */}
                <span className={`text-[10px] font-semibold transition-all duration-300 absolute bottom-1 ${isActive ? "opacity-100 text-primary translate-y-0" : "opacity-100 text-gray-500"}`}>
                  {isActive ? "•" : link.name}
                </span>
              </Link>
            );
